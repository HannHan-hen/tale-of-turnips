...
7. **No hand-drawn or licensed art we can't reproduce — all art is generated, keyed, and
   swappable.** Every texture is either drawn in code we own or generated by a model with the
   prompt committed, and is referenced only through `assetKeys` so gameplay never names a pixel
   or a file path. No stock art, no licensed image files, no uncredited downloads. Three routes
   are allowed:
   - **Procedural at boot (default):** drawn in code at boot via
     `src/game/assets/TextureFactory.ts`. This is the baseline for every slice and stays the
     fallback if a generated asset is ever missing.
   - **Code-baked raster (optional polish):** a checked-in generator script under `tools/`
     ...
     The generator — not just its output — lives in the repo, so the art is fully reproducible
     with `npm`/`python3` and no external service. See `tools/bake_sprites_poc.py`.
   - **Team-generated raster (image model):** the designer generates an image (e.g. ChatGPT
     GPT Image), commits the file under `src/assets/`, and records the model + exact prompt in
     `src/assets/SOURCES.md`. `BootScene` loads it under an `assetKey`. Best for backdrops,
     icons, portraits, and props; weakest at multi-frame character animation and pixel-exact
     tiles, so prefer procedural there. Post-process (trim/resize/palette-quantize) in code so
     the set stays coherent.

   Keep the set small, coherent, and swappable. No ugly placeholders.
