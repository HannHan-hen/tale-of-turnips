...
export class CombatController {
  ...
  spawn(spawns: EnemySpawn[]): void {
    for (const s of spawns) {
      ...
      const c = tileCenter(s.tile);
      const sprite = this.scene.add.image(c.x, c.y, def.textureKey).setOrigin(0.5, 0.9).setDepth(c.y);
      this.enemies.push({ def, sprite, hp: def.maxHp, x: c.x, y: c.y });
    }
  }
