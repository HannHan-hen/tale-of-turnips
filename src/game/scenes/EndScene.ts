...
export class EndScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add
      .rectangle(w / 2, h / 2, px(440), px(320), palette.uiPanel, 0.98)
      .setStrokeStyle(px(2), palette.outline);

    this.add
      .text(w / 2, px(44), '✦  The Ruin Heart is stilled  ✦', {
      ...
      .setOrigin(0.5);
    this.add
      .text(w / 2, px(70), 'The ruins fall quiet. The farm is safe.', {
        fontFamily: 'monospace',
        fontSize: fs(12),
    ...
    const left = w / 2 - px(196);
    const right = w / 2 + px(196);
    let y = px(104);
    for (const [label, value] of rows) {
      ...
      this.add
        .text(right, y, value, { fontFamily: 'monospace', fontSize: fs(13), color: toCss(palette.gold) })
        .setOrigin(1, 0);
      y += px(24);
    }

    this.add
      .text(w / 2, h - px(34), 'Press [Space] to return to your farm', {
        fontFamily: 'monospace',
        fontSize: fs(12),
        color: toCss(palette.uiInk),
      })
