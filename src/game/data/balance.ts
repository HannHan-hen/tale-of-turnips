...
// Pixel distances below are multiplied by the global render SCALE so the game feels identical
// (same tiles-per-second, same reach) when the world is scaled up. Tune the base numbers.
...
export const Balance = {
  ...
  playerSpeed: 130 * SCALE, // pixels per second
  interactRadius: 30 * SCALE, // how close (px) the player must be to interact
  autosaveMs: 4000, // how often the game autosaves

  // Combat (only matters in the ruins; hearts are global state)
  playerMaxHp: 5, // hearts
  attackOffset: 18 * SCALE, // px ahead of the player the swing lands
  attackRange: 26 * SCALE, // px radius of the swing
  attackDamage: 1,
  attackCooldownMs: 350,
  invulnMs: 900, // i-frames after taking a hit
  enemyAggroRange: 96 * SCALE, // px at which an enemy starts chasing
  enemyContactRange: 18 * SCALE, // px at which an enemy deals contact damage
  ...
  cropEatRange: 14 * SCALE, // px at which a raider devours a crop
